...
def generar_primo(numeros_de_bits):
    while True:
        primo = random.getrandbits(numeros_de_bits)
        if isprime(primo):
            return primo
...
def generar_claves(numeros_de_bits =1024):
    p = generar_primo(numeros_de_bits)
    q = generar_primo(numeros_de_bits)
    n = p * q
    phi_n = (p - 1) * (q - 1)
...
def main():
    # Generar claves
    clave_publica, clave_privada, p, q = generar_claves(numeros_de_bits)=1024
